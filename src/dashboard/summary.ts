...
export function getCategorySummaries(history: Movimiento[], companyName?: string) {
  ...
  // ⚡ Bolt Optimization: Using for...of instead of .filter().forEach() to
  // avoid allocating intermediate arrays for large dataset sizes.
...
export function getIncomeSummaries(history: Movimiento[]) {
  ...
  // ⚡ Bolt Optimization: Single loop avoids .filter() intermediate allocations
  for (const item of history) {
...
export function getIncomeTagSummaries(history: Movimiento[]) {
  ...
  // ⚡ Bolt Optimization: Loop fusion removes multiple O(n) array passes
  for (const item of history) {
    if (item.tipo !== 'ingreso') continue;
