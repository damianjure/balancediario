...
export function getCategorySummaries(history: Movimiento[], companyName?: string) {
  ...
  // Using a single for...of loop instead of chaining .filter().filter().forEach()
  // to prevent unnecessary intermediate array allocations for large datasets.
...
export function getIncomeSummaries(history: Movimiento[]) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Using a single for...of loop instead of chaining .filter().forEach()
  // to prevent unnecessary intermediate array allocations for large datasets.
  for (const item of history) {
