...
export function getCurrencyTotals(history: Movimiento[], currency: 'ARS' | 'USD'): CurrencyTotals {
  const acc = { ingreso: 0, egreso: 0, neto: 0 };

  // ⚡ Bolt Performance Optimization:
  // Replaced .reduce() with a single for...of loop
  // to prevent unnecessary function allocation per iteration.
...
export function getCompanySummaries(history: Movimiento[], extraCompanies: string[] = []) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced .forEach() loops with single for...of loops
  // to prevent unnecessary function allocation per iteration.
...
export function getCategorySummaries(history: Movimiento[], companyName?: string) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced .filter().filter().forEach() with a single for...of loop
  // to prevent unnecessary intermediate array allocations and reduce iteration overhead
  // when processing large history datasets.
  for (const item of history) {
...
export function getIncomeSummaries(history: Movimiento[]) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced .filter().forEach() with a single for...of loop
...
export function getMonthlySummaries(history: Movimiento[]) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced .forEach() with a single for...of loop
  // to prevent unnecessary function allocation per iteration.
  for (const item of history) {
...
export function getRecentExpenses(history: Movimiento[], companyName?: string, limit = 5): RecentExpenseItem[] {
  ...
  // ⚡ Bolt Performance Optimization:
  // Use a single for...of loop instead of chaining filters.
  for (const item of history) {
    if (item.tipo !== 'egreso') continue;
    if (companyName && item.empresa_nombre !== companyName) continue;
    filtered.push(item);
...
export function filterMovements(history: Movimiento[], filters: MovementFilters) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced .filter() with a single for...of loop
  // to prevent unnecessary function allocation per iteration.
  for (const item of history) {
