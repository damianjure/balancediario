...
if (bot) {
  ...
  bot.on("message:text", async (ctx) => {
    ...
    try {
      // Fetch current categories to help Gemini
      ...
      if (extracted.intent === "REGISTRAR" && extracted.items) {
        for (const item of extracted.items) {
          // Double check category or suggest fallback logic
  ...
  // Recurring Movements processing (Every day at 8 AM)
